...
/* RMBG-2.0 AI background removal. t2_rmbg_available() reports whether the
** library was built with RMBG support. t2_pipeline_set_rmbg() attaches an
** RMBG model to the pipeline; subsequent t2_generate calls use it for
** background removal instead of the simple solid-color heuristic.
** device: "auto", "cpu", "cuda", "vulkan". Returns 0 on success. */
TRELLIS2_CAPI int  t2_rmbg_available(void);
TRELLIS2_CAPI int  t2_pipeline_set_rmbg(t2_pipeline * p,
...
/* Dual-grid sidecar from fine-path generation (for standalone texturing). Valid after
** t2_generate on 512/1024 paths; empty on coarse. Buffers live until t2_mesh_free. */
TRELLIS2_CAPI int           t2_mesh_grid_res   (const t2_mesh_result * r);
TRELLIS2_CAPI int           t2_mesh_grid_nvox    (const t2_mesh_result * r);
TRELLIS2_CAPI const float * t2_mesh_grid_feats   (const t2_mesh_result * r); /* 7*nvox */
TRELLIS2_CAPI const int *   t2_mesh_grid_coords  (const t2_mesh_result * r); /* 3*nvox */

/* Standalone PBR texturing: mesh + reference image -> textured mesh.
** Pass grid_feats=NULL / grid_nvox=0 to derive the shape-encoder input via QEF
** (mesh -> dual grid, same path as dump_texture_reference.py). Otherwise pass
** the seven-channel decoded dual grid from t2_generate --save-grid.
** pipeline_type must be T2_PIPE_512 or T2_PIPE_1024 (grid_res 512 or 1024). */
TRELLIS2_CAPI t2_mesh_result * t2_texture_mesh(
    t2_pipeline * p,
