...
func (s *server) info() map[string]interface{} {
	...
	return map[string]interface{}{
		...
		"rmbg":              s.eng.HasRMBG() && s.eng.RMBGPath() != "",
...
func main() {
	...
	// RMBG-2.0 AI background removal: explicit path, auto-detect, or disabled.
	rmbgPath := *rmbgModel
	if rmbgPath == "" {
		rmbgPath = filepath.Join(*ggufDir, "rmbg_f16.gguf") // auto-detect candidate
	}
	if eng.HasRMBG() && rmbgPath != "" && fileExists(rmbgPath) {
		if err := eng.SetRMBG(rmbgPath, "auto"); err != nil {
			log.Printf("RMBG model load failed: %v", err)
		} else {
			log.Printf("RMBG-2.0 background removal enabled (%s)", rmbgPath)
		}
	} else if eng.HasRMBG() {
		log.Printf("RMBG-2.0 support compiled in, but no model file found (tried %s)", rmbgPath)
	}
