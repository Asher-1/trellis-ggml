...
type engine struct {
	...
	rmbgPath   string // RMBG-2.0 GGUF path (empty = disabled)
	rmbgDevice string // RMBG compute device ("auto", "cpu", "cuda", …)
	...
	rmbgAvailable func() int32
	setRmbg       func(p uintptr, gguf, device unsafe.Pointer, err unsafe.Pointer, errLen int32) int32
}
...
func newEngine(libPath, dinoGGUF, flowGGUF, decGGUF, slatGGUF, slatHRGGUF, shapeDecGGUF,
	...
	purego.RegisterLibFunc(&e.rmbgAvailable, lib, "t2_rmbg_available")
	purego.RegisterLibFunc(&e.setRmbg, lib, "t2_pipeline_set_rmbg")
...
func (e *engine) loadLocked() error {
	...
	// Re-attach the RMBG model after a lazy reload so every pipeline instance
	// sees the same configuration the caller originally requested.
	if e.rmbgPath != "" {
		if err := e.applyRmbg(e.rmbgPath, e.rmbgDevice); err != nil {
			return fmt.Errorf("rmbg re-attach: %w", err)
		}
	}
	return nil
}
...
// HasRMBG reports whether the library was built with RMBG-2.0 support.
func (e *engine) HasRMBG() bool {
	return e != nil && e.rmbgAvailable != nil && e.rmbgAvailable() != 0
}

// RMBGPath returns the configured RMBG model path (empty when disabled).
func (e *engine) RMBGPath() string {
	if e == nil {
		return ""
	}
	return e.rmbgPath
...
// SetRMBG attaches an RMBG-2.0 model to the pipeline for AI background removal.
// The model is used automatically by subsequent Generate calls. The path is
// remembered so that a lazy pipeline reload (unload-idle) re-attaches it.
func (e *engine) SetRMBG(ggufPath, device string) error {
	if !e.HasRMBG() {
		return fmt.Errorf("RMBG not available (library built without support)")
	}
	e.mu.Lock()
	defer e.mu.Unlock()
	e.rmbgPath = ggufPath
	e.rmbgDevice = device
	if e.pipeline == 0 {
		return nil // will be applied on next loadLocked()
	}
	return e.applyRmbg(ggufPath, device)
}

// applyRmbg calls the C setRmbg binding. e.mu must be held by the caller.
func (e *engine) applyRmbg(ggufPath, device string) error {
	if e.pipeline == 0 {
		return fmt.Errorf("pipeline not loaded")
	}
	errBuf := make([]byte, 512)
	cPath := append([]byte(ggufPath), 0)
	cDev := append([]byte(device), 0)
	rc := e.setRmbg(e.pipeline, unsafe.Pointer(&cPath[0]), unsafe.Pointer(&cDev[0]),
		unsafe.Pointer(&errBuf[0]), int32(len(errBuf)))
	if rc != 0 {
		return fmt.Errorf("set_rmbg: %s", cstr(errBuf))
	}
	return nil
}
