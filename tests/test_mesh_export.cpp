...
int main() {
    ...
    // Default export is UV atlas; vertex-colour is opt-in (legacy regression).
    setenv("T2GLB_VERTEX", "1", 1);
    ...
    // Default path: small mesh -> embedded baseColor + metallicRoughness textures.
    unsetenv("T2GLB_VERTEX");
    if (!t2glb::mesh_to_glb(verts, 4, tris, 4, pbr, opt, glb, err)) {
        std::fprintf(stderr, "atlas mesh_to_glb failed: %s\n", err.c_str());
        return 1;
    }
    const std::string atlas_bytes((const char *) glb.data(), glb.size());
    if (atlas_bytes.find("\"baseColorTexture\"") == std::string::npos ||
        atlas_bytes.find("\"metallicRoughnessTexture\"") == std::string::npos ||
        atlas_bytes.find("\"TEXCOORD_0\"") == std::string::npos) {
        std::fprintf(stderr, "atlas GLB missing embedded PBR textures or UVs\n");
        return 1;
    }
    if (atlas_bytes.find("\"COLOR_0\"") != std::string::npos) {
        std::fprintf(stderr, "atlas GLB should not use vertex COLOR_0\n");
        return 1;
    }
