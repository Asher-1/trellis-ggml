...
// stb_image implementations are compiled in stb_impl.cpp to avoid multiple
// definitions when linking with librmbg.a.
#include "stb_image.h"
...
bool run_texture_stage_core(t2_pipeline * p,
...
bool run_texture_stage_qef(t2_pipeline * p,
    ...
    std::fprintf(stderr, "QEF mesh->dual grid @ %d^3 (%d tris)...\n", grid, n_tris);
    ...
    std::fprintf(stderr, "QEF: %d active voxels\n", nvox);
...
int t2_rmbg_available(void) {
#ifdef TRELLIS2_HAVE_RMBG
    return trellis2_rmbg_available() ? 1 : 0;
#else
    return 0;
#endif
}

int t2_pipeline_set_rmbg(t2_pipeline * p, const char * rmbg_gguf,
                         const char * device, char * err, int err_len) {
    if (!p) { copy_err(err, err_len, "null pipeline"); return 1; }
#ifdef TRELLIS2_HAVE_RMBG
    trellis2_rmbg_free(p->rmbg);
    p->rmbg = nullptr;
    if (!rmbg_gguf || !rmbg_gguf[0]) return 0;  // clear
    std::string e;
    p->rmbg = trellis2_rmbg_load(rmbg_gguf, device ? device : "auto", &e);
    if (!p->rmbg) { copy_err(err, err_len, e); return 1; }
    return 0;
#else
    (void)rmbg_gguf; (void)device;
    copy_err(err, err_len, "library built without RMBG support");
    return 1;
#endif
}

// Detect the WebP "RIFF....WEBP" signature in the first 12 bytes.
static bool is_webp(const uint8_t * data, int len) {
    return len >= 12
...
static int preprocess_image_bytes_mode(const void * image_bytes, int image_len,
    ...
    const auto * bytes = (const uint8_t *) image_bytes;
...
#ifdef TRELLIS2_HAVE_WEBP
    if (is_webp(bytes, image_len)) {
        from_webp = true;
        int wp = 0, hp = 0;
        if (!WebPGetInfo(bytes, (size_t) image_len, &wp, &hp)) {
            copy_err(err, err_len, "WebP probe failed");
            return 1;
        }
        w = wp; h = hp;
        ...
        px = WebPDecodeRGBA(bytes, (size_t) image_len, &wp, &hp);
        if (!px) {
            copy_err(err, err_len, "WebP decode failed");
            return 1;
        }
        comp = 4;  // WebPDecodeRGBA always returns 4 channels
    } else
#endif
    {
        if (!stbi_info_from_memory(bytes, image_len, &w, &h, &comp)) {
        ...
        px = stbi_load_from_memory(bytes, image_len, &w, &h, &comp, 4);
        if (!px) {
...
t2_mesh_result * t2_generate(t2_pipeline * p,
    ...
    // When the pipeline has an RMBG model, run AI background removal first.
    const void * preproc_bytes = image_bytes;
    int preproc_len = image_len;
    std::vector<uint8_t> rmbg_png;
#ifdef TRELLIS2_HAVE_RMBG
    if (p->rmbg) {
        int iw = 0, ih = 0, ic = 0;
        unsigned char * ipx = stbi_load_from_memory(
            (const stbi_uc *)image_bytes, image_len, &iw, &ih, &ic, 4);
        if (!ipx) {
            copy_err(err, err_len, "RMBG: image decode failed");
            return nullptr;
        }
        ...
        int rc = trellis2_rmbg_remove_background(
            p->rmbg, ipx, iw, ih, &rmbg_out, &rmbg_out_len, &rmbg_err);
        stbi_image_free(ipx);
        if (rc != 0 || !rmbg_out) {
            copy_err(err, err_len, "RMBG: " + rmbg_err);
            return nullptr;
        }
        rmbg_png.assign(rmbg_out, rmbg_out + rmbg_out_len);
        trellis2_rmbg_free_buffer(rmbg_out);
        // Debug: save RMBG output for verification
        if (const char * dbg = std::getenv("T2_DEBUG_SAVE_RMBG")) {
            FILE * f = std::fopen(dbg, "wb");
            if (f) { std::fwrite(rmbg_png.data(), 1, rmbg_png.size(), f); std::fclose(f); }
        }
        preproc_bytes = rmbg_png.data();
        preproc_len = (int)rmbg_png.size();
        background_mode = T2_BACKGROUND_KEEP;
    }
#endif

    if (preprocess_image_bytes_mode(preproc_bytes, preproc_len, S, rgb.data(),
...
int t2_mesh_grid_res(const t2_mesh_result * r) { return r ? r->grid_res : 0; }
int t2_mesh_grid_nvox(const t2_mesh_result * r) {
    return r ? (int) (r->grid_coords.size() / 3) : 0;
}
const float * t2_mesh_grid_feats(const t2_mesh_result * r) {
    return (r && !r->grid_feats.empty()) ? r->grid_feats.data() : nullptr;
}
const int * t2_mesh_grid_coords(const t2_mesh_result * r) {
    return (r && !r->grid_coords.empty()) ? r->grid_coords.data() : nullptr;
}

t2_mesh_result * t2_texture_mesh(
    t2_pipeline * p,
    const float * verts, int n_verts,
    const int * tris, int n_tris,
    const float * grid_feats, int grid_nvox,
    const int * grid_coords, int grid_res,
    int pipeline_type,
    const void * image_bytes, int image_len,
    int background_mode,
    uint64_t seed, int texture_steps,
    t2_progress_fn progress, void * user,
    char * err, int err_len) {
    if (!p) { copy_err(err, err_len, "null pipeline"); return nullptr; }
    if (!p->texture) { copy_err(err, err_len, "texture models not loaded"); return nullptr; }
    if (!verts || n_verts <= 0 || !tris || n_tris <= 0) {
        copy_err(err, err_len, "invalid mesh"); return nullptr;
    }
    const bool use_qef = !grid_feats || grid_nvox <= 0 || !grid_coords;
    if (!use_qef && grid_res <= 0) {
        copy_err(err, err_len, "invalid dual grid"); return nullptr;
    }
    if (use_qef && grid_res <= 0) {
        copy_err(err, err_len, "grid_res required for QEF texturing"); return nullptr;
    }
    if (!image_bytes || image_len <= 0) {
        copy_err(err, err_len, "invalid image"); return nullptr;
    }
    int pt = pipeline_type;
    if (pt != T2_PIPE_512 && pt != T2_PIPE_1024) {
        copy_err(err, err_len, "pipeline_type must be 512 or 1024"); return nullptr;
    }
    if (pt == T2_PIPE_1024 && p->texflow_hr_path.empty()) {
        copy_err(err, err_len, "1024 texture flow model not loaded"); return nullptr;
    }

    std::string e;
    const int S = (pt == T2_PIPE_1024) ? 1024 : 512;
    if (progress) progress(user, T2_STAGE_PREPROCESS, 0, 0);
    std::vector<unsigned char> rgb((size_t) S * S * 3);
    char perr[256] = {0};

    const void * tex_preproc_bytes = image_bytes;
    int tex_preproc_len = image_len;
    std::vector<uint8_t> tex_rmbg_png;
#ifdef TRELLIS2_HAVE_RMBG
    if (p->rmbg) {
        int iw = 0, ih = 0, ic = 0;
        unsigned char * ipx = stbi_load_from_memory(
        ...
        tex_rmbg_png.assign(rmbg_out, rmbg_out + rmbg_out_len);
        trellis2_rmbg_free_buffer(rmbg_out);
        tex_preproc_bytes = tex_rmbg_png.data();
        tex_preproc_len = (int)tex_rmbg_png.size();
        background_mode = T2_BACKGROUND_KEEP;
    }
#endif

    if (preprocess_image_bytes_mode(tex_preproc_bytes, tex_preproc_len, S, rgb.data(),
                                    background_mode, perr, sizeof(perr))) {
        copy_err(err, err_len, perr);
        return nullptr;
    }
    ...
    if (use_qef) {
        ...
        if (!run_texture_stage_qef(p, verts, n_verts, tris, n_tris, grid_res,
                                   r->verts, pt, cond, seed, texture_steps,
                                   progress, user, pbr, qef_coords, e)) {
        ...
        std::vector<float> dec_feats(r->grid_feats.begin(), r->grid_feats.end());
        std::vector<int32_t> dec_coords(r->grid_coords.begin(), r->grid_coords.end());
        if (!run_texture_stage(p, dec_feats, dec_coords, r->verts, grid_res, pt,
                               cond, seed, texture_steps, progress, user, pbr, e)) {
            copy_err(err, err_len, "texture: " + e);
            delete r;
            return nullptr;
        }
