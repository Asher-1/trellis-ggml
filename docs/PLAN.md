...
RMBG-2.0 (BiRefNet) background removal is now integrated via the
`third_party/RMBG-2.0-GGML` submodule (see README "RMBG-2.0 AI background
removal"). The port also includes deterministic removal of near-black/near-white
backgrounds connected to the image border, with soft alpha edges and browser
controls to force or disable it. Texture and cascade status is tracked below.
...
- **Background removal — DONE.** BiRefNet/RMBG-2.0 is integrated via the
  `third_party/RMBG-2.0-GGML` submodule (see README). The demo can also
  instruct a transparent PNG and use the image as-is otherwise.
...
- **Quantized shipping variants / benchmarks — DONE.** Q8 conversion
  (`quantize_to_q8.py`), a benchmark table (`docs/BENCHMARK.md`), and end-to-end
  Q8 parity are complete; f16 remains the demo default.
